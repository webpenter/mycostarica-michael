<?php
if(!function_exists('homeyAvailabilityCalendarDayDate')) {
    function homeyAvailabilityCalendarDayDate() {
        ...
        while( $numberOfMonths <= homey_calendar_months() ) {
            ...
            homeyGenerateMonthDayDate( $numberOfMonths, $unixMonth, $currentMonth, $currentYear );
...
if(!function_exists('homeyGenerateMonthDayDate')) {
    function homeyGenerateMonthDayDate( $numberOfMonths, $unixMonth, $currentMonth, $currentYear ) {
        ...
        for ( $day = 1; $day <= $daysInMonth; ++$day ) {
            ...
            if( array_key_exists($timestamp, $bookedDays) ) {
                $calendar_day_class = ' day-booked homey-not-available-for-booking day-date-booked';
                
            } elseif( array_key_exists($timestamp, $pending_dates) ) {
                $calendar_day_class = 'day-pending day-date-pending  homey-not-available-for-booking';
               
            } elseif(array_key_exists($timestamp, $unavailable_dates)) {
                $calendar_day_class = 'day-unavailable day-date-unavailable homey-not-available-for-booking';
            } else {
                if( $timestamp < (time()-24*60*60) ) {
                    $dayClass = "day-disabled past-day";
                } else {
                    $dayClass = "future-day";
                    $calendar_day_class = 'day-available day-date-available';
                }
            }
