...
class homey_Flickr_Feeds extends WP_Widget {
	...
	**/
	public function __construct() {
		...
		parent::__construct(
	 		'homey_flickr_feeds', // Base ID
			esc_html__( 'Homey: Flickr', 'homey' ), // Name
			array( 'description' => esc_html__( 'Show photos from Flickr.', 'homey' ), ) // Args
		);
	...
	function form($instance)
	{
	 ...
	 ?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'userid' ) ); ?>"><?php esc_html_e( 'Flickr ID', 'homey' ); ?>:</label> <small><a href="http://idgettr.com/" target="_blank"><?php esc_html_e( 'What\'s my Flickr ID?', 'homey' ); ?></a></small>
			...
			<small class="howto"><?php esc_html_e( 'Example ID: 23100287@N07', 'homey' ); ?></small>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php esc_html_e( 'Number of photos', 'homey' ); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 't_width' ) ); ?>"><?php esc_html_e( 'Thumbnail width', 'homey' ); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 't_height' ) ); ?>"><?php esc_html_e( 'Thumbnail height', 'homey' ); ?>:</label>
...
if ( ! function_exists( 'homey_Flickr_Feeds_loader' ) ) {
    function homey_Flickr_Feeds_loader (){
     register_widget( 'homey_Flickr_Feeds' );
    }
     add_action( 'widgets_init', 'homey_Flickr_Feeds_loader' );
}
?>