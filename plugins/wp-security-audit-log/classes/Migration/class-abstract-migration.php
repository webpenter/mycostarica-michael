...
if ( ! class_exists( '\WSAL\Utils\Abstract_Migration' ) ) {
	...
	class Abstract_Migration {
		...
		public static function migrate() {
			...
			if ( class_exists( 'Metadata_Migration_440' ) && ! empty( WP_Helper::get_global_option( Metadata_Migration_440::OPTION_NAME_MIGRATION_INFO, array() ) ) ) {

				new Metadata_Migration_440( 'external' );
				new Metadata_Migration_440( 'archive' );

				add_action( 'all_admin_notices', array( 'Metadata_Migration_440', 'maybe_display_progress_admin_notice' ) );
			}
