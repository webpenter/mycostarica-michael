...
class PluginTools
{
	...
	public function export_settings()
	{
		...
		header( 'Content-Disposition: attachment; filename=' . str_replace( '/', '-', $output ) . '-wpar-export-' . date( 'm-d-Y' ) . '.json' );
		header( "Expires: 0" );
	...
	public function remove_settings()
	{
		...
		delete_option( 'wpar_global_last_run' );
	...
	public function remove_metas()
	{
		...
		$this->do_action( 'remove_post_metadata' );
	...
	public function deschedule_posts()
	{
		...
		$this->do_action( 'deschedule_posts' );
	...
	public function admin_notice()
	{
    	if ( get_transient( 'wpar_import_db_done' ) !== false ) { ?>
			<div class="notice notice-success is-dismissible"><p><strong><?php _e( 'Success! Plugin Settings has been imported successfully.', 'wp-auto-republish' ); ?></strong></p></div><?php 
