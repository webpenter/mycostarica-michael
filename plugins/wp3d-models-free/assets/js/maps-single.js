...
if(jQuery('#wp3d-single-map').attr("data-latlng")){ // check for attribute 'data-latlng'

 // ************ SET SOME VARS ************* //
    
    var location = {};
        location.lat = parseFloat(jQuery('#wp3d-single-map').attr("data-latlng").split(",")[0]);
        location.lon = parseFloat(jQuery('#wp3d-single-map').attr("data-latlng").split(",")[1]);
        locations.push(location);

    // ZOOM LEVEL
    if(jQuery('#wp3d-single-map').attr("data-map-zoom")){
        var wp3dmapzoom = parseInt(jQuery('#wp3d-single-map').attr("data-map-zoom"));
    } else {
        var wp3dmapzoom = 6;
    }
    
    // MAP TYPE
    if(jQuery('#wp3d-single-map').attr("data-map-type")){
        var wp3dmaptype = jQuery('#wp3d-single-map').attr("data-map-type");
    } else {
 ...
 // ************ GOOGLE MAP OBJECT ************* //
    ...
    var map;
    var gmarkers = [];
    var myLatlng = new google.maps.LatLng(location.lat,location.lon); // single location
    
    var dragtoggle;
    if (viewport.width > 768) { dragtoggle = true; } else { dragtoggle = false; };
    
    // Default map options for how to display the Google map
    var mapOptions = { 
    ...
    map = new google.maps.Map(document.getElementById("wp3d-single-map"), mapOptions);
    ...
    // ************ LOOP THROUGH AND PLACE MARKERS ************* //
        ...
        for(var i = 0; i < locations.length; i++) 
        {
            ...
            var description = "<div class=\"model-infowindow\">" +
                ...
                "<a href=\"/3d-model/" + locations[ i ].slug + "\" class=\"infowindow-btn\">VIEW</a>" + 
                "</div>";
            
            // PIN/MARKER TYPE
            if(jQuery('#wp3d-single-map').attr("data-marker-type") == 'stock'){
                ...
                var marker = new MarkerWithLabel( { 
                    ...
                    labelClass: "wp3d-pin-single", // the CSS class for the label                
                });
            }             
    ...
    } // end check for attribute 'data-latlng'
