		...
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
		   ...
		   && isset( $_GET['puc_slug'] ) && ($_GET['puc_slug'] == $this->slug) ) {
		    ...
		    printf(
		     ...
		     apply_filters( 'puc_manual_check_error_msg-' . $this->slug, stripslashes_deep($_GET['puc_error_message']), 1 )
		    );
		  }
	}
...
class PluginInfo_2_0 {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		if ( empty($apiResponse) || !is_object($apiResponse) ){
			if ( $triggerErrors ) {
				trigger_error(
					"Failed to parse plugin metadata. Try validating your .json file with " . esc_url( "http://jsonlint.com/" ),
					E_USER_NOTICE
				);
			}
			return null;
