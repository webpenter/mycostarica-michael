	...
	function display_scheduling_posts() {
		...
		if( !empty( $data ) ) {
			...
			foreach ($data as $key => $value){
				...
				$data[$key]['post_title'] 	= '<a target="_blank" href="'.esc_url($edit_link).'">' . esc_html($value['post_title']) . '</a>';
    ...
    function extra_tablenav( $which ) {
    	...
    	if( $which == 'top' ) {
    		...
    		$html .= '<div class="wp-auto-date-filter ' . esc_attr($style) . '">
							...
							<input type="text" name="wpw_auto_start_date" id="wpw_auto_start_date" class="wpw-auto-datepicker" placeholder="'.esc_html__( 'From Date', 'wpwautoposter') .'" value="'.(isset($_REQUEST['wpw_auto_start_date'])?  $_REQUEST['wpw_auto_start_date'] : '').'">
							<input type="text" name="wpw_auto_end_date" id="wpw_auto_end_date" class="wpw-auto-datepicker" placeholder="'.esc_html__( 'To Date', 'wpwautoposter') .'" value="'.(isset($_REQUEST['wpw_auto_end_date'])?  $_REQUEST['wpw_auto_end_date'] : '').'">
					</div>';
...
<div class="wrap wpw-scheduling-wrap">
	...
	<h2><?php esc_html_e( 'Manage Schedules', 'wpwautoposter' ); ?></h2>
	...
	<h2 class="nav-tab-wrapper wpw-auto-poster-h2 wpw-auto-poster-h2_common">
...
<?php if( !empty($wpw_auto_poster_options) && $wpw_auto_poster_options['schedule_wallpost_option'] == "hourly") { 
	...
	<div class="wpw-auto-popup-content wpw-auto-schedule-content">
		...
		<div class="wpw-auto-popup-header">
			...
			<div class="wpw-popup-close"><a href="javascript:void(0);" class="wpw-close-button"><img src="<?php echo esc_url(WPW_AUTO_POSTER_IMG_URL); ?>/tb-close.png" alt="Close"></a></div>
		</div>
